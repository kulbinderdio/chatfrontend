...
class ProfilesViewTests: XCTestCase {
    ...
    func testProfileDeletion() {
        ...
        guard let secondProfileId = secondProfile?.id else {
            XCTFail("Second profile ID is nil")
            return
        }
    ...
    func testDefaultProfileSetting() {
        ...
        var profile1: ModelProfile?
        var profile2: ModelProfile?
        
        XCTAssertNoThrow(profile1 = try profileManager.createProfile(
            name: profile1Name,
        ...
        XCTAssertNoThrow(profile2 = try profileManager.createProfile(
            name: profile2Name,
            apiEndpoint: URL(string: "https://api.example.com")!,
            apiKey: "api-key-2",
            modelName: "model-2",
        ...
        guard let profile1Id = profile1?.id, let profile2Id = profile2?.id else {
            XCTFail("Profile IDs are nil")
            return
        }
        
        // Set profile 2 as default
        XCTAssertNoThrow(try profileManager.setDefaultProfile(id: profile2Id))
    ...
    func testProfileDuplication() {
        ...
        var originalProfile: ModelProfile?
        XCTAssertNoThrow(originalProfile = try profileManager.createProfile(
            name: name,
            apiEndpoint: apiEndpoint,
            apiKey: apiKey,
            modelName: modelName,
        ...
        guard let profileId = originalProfile?.id else {
            XCTFail("Profile ID is nil")
            return
        }
